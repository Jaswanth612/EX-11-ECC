...
## DESIGN STEPS:
Step 1: Choose a large prime number p and an elliptic curve defined by the equation 𝑦^2 = 𝑥^3 + 𝑎𝑥 +𝑏 mod p along with a base point G on the curve.
...
int main() {
    ...
    printf("Enter Jas's private key: ");
    scanf("%lld", &privateA);
    printf("Enter Jass's private key: ");
    scanf("%lld", &privateB);
    ...
    printf("Jas's public key: (%lld, %lld)\n", publicA.x, publicA.y);
    printf("Jass's public key: (%lld, %lld)\n", publicB.x, publicB.y);
    ...
    printf("Shared secret computed by Jas: (%lld, %lld)\n", sharedSecretA.x, sharedSecretA.y);
    printf("Shared secret computed by Jass: (%lld, %lld)\n", sharedSecretB.x, sharedSecretB.y);
...
![image](https://github.com/user-attachments/assets/62b74fab-ceed-4681-8b3c-7ff19f437f66)


## Result:
The implement the Elliptic Curve Cryptography (ECC) key exchange algorithm and securely derive a shared secret key executed sucessfully
